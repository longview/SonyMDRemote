...
namespace SonyMDRemote
{
    public partial class SonyMDRemote
    {
        ...
        readonly byte[] MDS_TX_ReqTrackRecordDate = new byte[] { 0x20, 0x24 }; // next byte is track number
        readonly byte[] MDS_TX_ReqTOCData = new byte[] { 0x20, 0x44, 0x01 };
        readonly byte[] MDS_TX_ReqTrackTime = new byte[] { 0x20, 0x45, 0x01 }; // next byte is track number
        readonly byte[] MDS_TX_ReqDiscName = new byte[] { 0x20, 0x48, 0x01 };
        readonly byte[] MDS_TX_ReqTrackName = new byte[] { 0x20, 0x4A }; // next byte is track number
        readonly byte[] MDS_TX_ReqDiscAndTrackNames = new byte[] { 0x20, 0x4C, 0x01 };
        readonly byte[] MDS_TX_ReqRemainingRecordTime = new byte[] { 0x20, 0x54, 0x01 };
        readonly byte[] MDS_TX_ReqTrackRemainingNameSize = new byte[] { 0x20, 0x55, 0x00 }; // next byte is track number
        ...
        // final namedata byte must be 0 to effect write
        // then finally WRITE COMPLETE
        readonly byte[] MDS_TX_WriteTrackName1 = new byte[] { 0x20, 0x72 }; // next byte is track number, next 16 bytes is name data, null terminated
        readonly byte[] MDS_TX_WriteTrackName2 = new byte[] { 0x20, 0x73 }; // next byte is packet number, next 16 bytes is name data, null terminated

        // basic transport controls
        readonly byte[] MDS_TX_Play = new byte[] { 0x02, 0x01 };
        readonly byte[] MDS_TX_Stop = new byte[] { 0x02, 0x02 };
        readonly byte[] MDS_TX_PlayPause = new byte[] { 0x02, 0x03 };
        readonly byte[] MDS_TX_Pause = new byte[] { 0x02, 0x06 };
        readonly byte[] MDS_TX_FFW_REW_OFF = new byte[] { 0x00 };
        readonly byte[] MDS_TX_Rewind = new byte[] { 0x02, 0x13 };
        readonly byte[] MDS_TX_FastForward = new byte[] { 0x02, 0x14 };
        readonly byte[] MDS_TX_NextTrack = new byte[] { 0x02, 0x16 };
        readonly byte[] MDS_TX_PrevTrack = new byte[] { 0x02, 0x15 };
        readonly byte[] MDS_TX_RecordArm = new byte[] { 0x02, 0x21 };
        readonly byte[] MDS_TX_Eject = new byte[] { 0x02, 0x40 };
        readonly byte[] MDS_TX_AutoPauseOn = new byte[] { 0x02, 0x81 };
        readonly byte[] MDS_TX_AutoPauseOff = new byte[] { 0x02, 0x80 };

        // from addendum
        readonly byte[] MDS_TX_RepeatOff = new byte[] { 0x02, 0xA0 };
        readonly byte[] MDS_TX_RepeatAll = new byte[] { 0x02, 0xA1 };
        readonly byte[] MDS_TX_Repeat1Tr = new byte[] { 0x02, 0xA2 };

        // track management
        readonly byte[] MDS_TX_StartPlayAtTrack = new byte[] { 0x03, 0x42, 0x01 }; // next byte is track number
        readonly byte[] MDS_TX_PausePlayAtTrack = new byte[] { 0x03, 0x43, 0x01 }; // next byte is track number, pauses at the start of specific track

        readonly byte[] MDS_TX_EnableElapsedTimeTransmit = new byte[] { 0x07, 0x10 };
        readonly byte[] MDS_TX_DisableElapsedTimeTransmit = new byte[] { 0x07, 0x11 };
            ...
            string discname, TimeSpan disclength, TimeSpan remainingrecordtime, string outputfilename)
        {
            try
            {
                ...
                {
                    TimeSpan ts;
                    if (!tracklengths.TryGetValue(track.Key, out ts))
                        ts = new TimeSpan(0);

                    outputfile.WriteLine(String.Format("{0}\t{1}\t{2}", track.Key, track.Value.ToString(), ts.ToString()));
                }

                outputfile.Close();

                AppendLog("Exported track listing to file {0}.", outputfilename);

                return true;
            }
            catch (Exception e)
            {
                AppendLog("Save error: {0}, type {1}", e.Message, e.ToString());
                return false;
            }
        ...
        private bool ImportTrackListing(ref IDictionary<int, StringBuilder> tracknames, ref IDictionary<int, TimeSpan> tracklengths,
            ref string discname, ref TimeSpan disclength, ref TimeSpan remainingrecordtime, string inputfilename)
        {
            try
            {
                ...
                AppendLog("Read from file header " + firstline);
                ...
                if (discinfo.Length > 1)
                    disclength = TimeSpan.Parse(discinfo[1]);
                if (discinfo.Length > 2)
                    remainingrecordtime = TimeSpan.Parse(discinfo[2]);
                ...
                while ((line = inputfile.ReadLine()) != null)
                {
                    ...
                    {
                        tracknames.Add(tracknumber, new StringBuilder(trackinfo[1]));
                        if (trackinfo[2] != "00:00:00")
                            tracklengths.Add(tracknumber, TimeSpan.Parse(trackinfo[2]));
                    }
                ...
                AppendLog("Imported track listing from file {0}.", inputfilename);

                return true;
            }
            catch (Exception e)
            {
                AppendLog("Open error: {0}, type {1}", e.Message, e.ToString());
                return false;
            }
